package com.jdbccoverage.cli

import com.jdbccoverage.detector.ImplementationDetector
...
import com.jdbccoverage.parser.SourceParser
import com.jdbccoverage.profile.DriverProfile
import com.jdbccoverage.profile.ProfileResolver
import com.jdbccoverage.report.console.ConsoleReporter
import com.jdbccoverage.report.json.JsonReporter
import com.jdbccoverage.resolver.JdbcInterfaceResolver
import com.jdbccoverage.spec.JdbcSpecLoader
...
class JdbcCoverageCommand : Runnable {
    override fun run() {
...
internal fun runAnalysis(
    ...
    maxVersion: com.jdbccoverage.model.JdbcVersion? = null,
): AnalysisReport? {
...
class AnalyzeCommand : Callable<Int> {
    ...
    override fun call(): Int {
        ...
        val maxVersion = jdbcVersion?.let { requested ->
            com.jdbccoverage.model.JdbcVersion.fromString(requested) ?: run {
                System.err.println(
                    "Error: Unknown JDBC version: $requested " +
                        "(known: ${com.jdbccoverage.model.JdbcVersion.entries.joinToString { it.display }})",
                )
                return 1
            }
        }
        ...
        historyDir?.let { dir ->
            val entry = com.jdbccoverage.history.HistoryRecorder(dir).record(report)
            val slug = com.jdbccoverage.history.HistoryRecorder.slugOf(report.driverName)
...
fun main(args: Array<String>) {
    val exitCode = CommandLine(JdbcCoverageCommand()).execute(*args)
